...
export const getComplaintStats = async (req, res) => {
    try {
        const [statusRows] = await pool.query(`SELECT status, COUNT(*) as count FROM complaints WHERE is_deleted = 0 GROUP BY status`);
        const [[{ total }]] = await pool.query(`SELECT COUNT(*) as total FROM complaints WHERE is_deleted = 0`);
        const stats = { total };
    ...
    try {
        ...
        if (shouldSendSMS && phone && phone.trim()) {
            ...
            await pool.query(
                ...
                ['Complaint', newId, 'SMS', phone.trim(), smsBody, commAdminId]
        ...
        if (shouldSendEmail && email && email.trim()) {
            ...
            await pool.query(
                `INSERT INTO communications_logs (entity_type, entity_id, channel, recipient, message, admin_user_id) VALUES (?, ?, ?, ?, ?, ?)`,
                ['Complaint', newId, 'Email', email.trim(), emailBody, commAdminId]
            ).catch(err => console.warn('[Log failed]', err.message));
        }
